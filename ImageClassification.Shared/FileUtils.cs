...
namespace ImageClassification
{
    public class FileUtils
    {
        ...
        public static IEnumerable<(string imagePath, string label)> LoadImagesFromListedFilePaths(
           List<string> imagePaths,
           string label)
        {

          return  imagePaths.Select(imagePath =>
            {
                for (var index = 0; index < label.Length; index++)
                {
                    if (!char.IsLetter(label[index]))
        ...
        public static IEnumerable<InMemoryImageData> LoadInMemoryImagesFromDirectory(
            ...
            => LoadImagesFromDirectory(folder, useFolderNameAsLabel)
                .Select(x => new InMemoryImageData(
                    ...
                    imageFilePath: Path.GetDirectoryName(x.imagePath)
                    ));
